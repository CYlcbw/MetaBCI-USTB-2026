...
本项目面向空间认知能力训练与脑机接口主动控制应用，构建基于MetaBCI的生态化VR空间认知训练系统。系统以四分类运动想象（MI）脑电识别为核心，将受试者的运动想象脑电信号解码为前、后、左、右四类方位控制指令，并通过 Socket 通讯驱动 VR 交互训练游戏中的虚拟角色完成移动与目标捕获任务。
