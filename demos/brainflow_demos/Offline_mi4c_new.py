...
def get_data(srate, subjects, pick_chs):
    ...
    if MetaBCIData is None:
        raise ImportError("MetaBCIData is unavailable because demos/brainflow_demos/datasets.py is not present")
...
def get_data_new(subjects):
    ...
    train_feature_arr = np.asarray(segmented_data)
    train_label_arr = np.asarray(label_run_expanded)
...
def offline_validation_new(X, y, subject, X_test=None, y_test=None):
    ...
    batch_size = 64
    kfolds = 5
    model_savePath = "checkpoints\\{}\\Subject_0{}".format(model_name+"_ustb2026mi4c", str(subject[0]))
    ...
    #                             X_train, y_train, kfolds, device,
    #                             model_name, subject, model_savePath)
    features_path = "checkpoints\\{}\\Subject_0{}\\visualization\\tsne_feature\\".format(model_name+"_ustb2026mi4c", str(subject[0]))
