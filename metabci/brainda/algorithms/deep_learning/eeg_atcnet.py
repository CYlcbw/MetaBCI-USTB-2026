...
class ATCNet(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        sw_outputs = []
        active_windows = min(self.n_windows, x.shape[2])
        for i in range(active_windows):
            st = i
            end = x.shape[2] - active_windows + i + 1
        ...
        if self.fuse == 'average':
            ...
            if active_windows < self.n_windows:
                pad_width = self.fc_layer.in_features - sw_concat.shape[1]
                sw_concat = torch.nn.functional.pad(sw_concat, (0, pad_width))
